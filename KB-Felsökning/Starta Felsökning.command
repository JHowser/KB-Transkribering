#!/bin/bash
# Dubbelklicka denna fil för att starta KB Transkribering & Felsökning.
cd "$(dirname "$0")" || exit 1
clear
echo "──────────────────────────────"
echo "  KB Transkribering & Felsökning"
echo "──────────────────────────────"
...
if [ ! -d ".venv" ]; then
  ...
  ./.venv/bin/pip install flask pywhispercpp av numpy certifi pillow || { echo "  Installationen misslyckades."; read -r; exit 1; }
  echo "  Klart."
fi

# Säkerställ nödvändiga paket även om miljön redan fanns sedan tidigare.
